...
def main():
    ...
    ok(immo_api._monat_soll(t2, 2026, 6) == 288.0, f"Juni OHNE Vereinbarung = 288.00 (Warmmiete 540 × 16/30) — got {immo_api._monat_soll(t2,2026,6)}")
    ...
    ok(immo_api._monat_soll(t2, 2026, 7) == 540, f"Juli = 540 (voller Monat Warmmiete) — got {immo_api._monat_soll(t2,2026,7)}")
    ok(immo_api._soll_faellig(t2, 2026) == 3510, f"Soll 2026 = 3510 (Erstmiete 270 + 6×540) — got {immo_api._soll_faellig(t2,2026)}")
    L.ensure_sollbuchungen(db, 1, 2026); db.commit()
    ...
    db.refresh(t2)
    ok(immo_api._monat_soll(t2, 2026, 6) == 288.0, f"Löschen (-1) → zurück zu Tagesanteil 288.00 — got {immo_api._monat_soll(t2,2026,6)}")
