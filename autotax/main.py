...
# Additive, flag-gated Kasa MVP router (FEAT_KASSE_V2, default OFF -> 404).
try:
    from autotax.kasse_api import router as _kasse_v2_router
    app.include_router(_kasse_v2_router)
    logger.info("kasse_v2 router registered (flag-gated, default OFF)")
except Exception as _e:  # pragma: no cover - defensive
...
# --- Privacy/legal HTML extracted to autotax/legal.py (Phase 2.2, 2026-05-27) ---
# Constants + _privacy_page() multilang builder artik autotax.legal icinde.
# Route handler'lari (asagidaki @app.get("/datenschutz") vb.) bunlari kullanir.
from autotax.legal import (
    _PRIVACY_CSS, _PRIVACY_LANG_BAR, _PRIVACY_THIRD_PARTIES,
    _privacy_page,
)
...
def auth_reset_page(token: str = Query(...)):
    ...
    set a new password, POSTs to /auth/reset-confirm. Standalone (no SPA)."""
    safe_token = token.replace('"', '').replace("'", "").replace("<", "").replace(">", "")
    return HTMLResponse(content=f"""<!DOCTYPE html><html lang="de"><head><meta charset="UTF-8">
<title>Passwort zurücksetzen — AutoTax</title>
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<style>
body{{font-family:'DM Sans',Arial,sans-serif;background:#050a12;color:#e8edf5;margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}}
.card{{background:#0f172a;border:1px solid #2a3548;border-radius:14px;padding:32px;width:100%;max-width:420px}}
h1{{margin:0 0 8px;color:#10b981;font-size:24px}}
...
input{{width:100%;padding:12px;background:#1a2234;border:1px solid #2a3548;border-radius:8px;color:#e8edf5;font-size:14px;box-sizing:border-box}}
input:focus{{outline:none;border-color:#10b981}}
button{{width:100%;padding:12px;background:#10b981;color:#fff;border:none;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer;margin-top:16px}}
button:disabled{{opacity:0.6;cursor:not-allowed}}
.hint{{font-size:11px;color:#64748b;margin-top:4px}}
.msg{{padding:12px;border-radius:8px;font-size:13px;margin-bottom:16px}}
.err{{background:rgba(239,68,68,0.1);color:#f87171;border:1px solid rgba(239,68,68,0.3)}}
.ok{{background:rgba(16,185,129,0.1);color:#10b981;border:1px solid rgba(16,185,129,0.3)}}
</style></head><body>
<div class="card">
<h1>Neues Passwort setzen</h1>
<p>Bitte gib dein neues Passwort ein. Du wirst danach automatisch zur Anmeldung weitergeleitet.</p>
<div id="msg"></div>
<label for="pw">Neues Passwort</label>
<input id="pw" type="password" placeholder="min. 8 Zeichen, 1 Großbuchstabe, 1 Zahl" autocomplete="new-password"/>
<div class="hint">Min. 8 Zeichen · 1 Großbuchstabe · 1 Zahl</div>
<label for="pw2" style="margin-top:16px">Passwort bestätigen</label>
<input id="pw2" type="password" placeholder="Nochmal eingeben" autocomplete="new-password"/>
<button id="submit">Passwort speichern</button>
</div>
<script>
...
btn.onclick=async()=>{{
  ...
  if(pw.length<8){{msg.className="msg err";msg.textContent="Passwort min. 8 Zeichen.";return;}}
  if(!/[A-Z]/.test(pw)){{msg.className="msg err";msg.textContent="Passwort muss 1 Großbuchstabe enthalten.";return;}}
  if(!/[0-9]/.test(pw)){{msg.className="msg err";msg.textContent="Passwort muss 1 Zahl enthalten.";return;}}
  if(pw!==pw2){{msg.className="msg err";msg.textContent="Passwörter stimmen nicht überein.";return;}}
  btn.disabled=true;btn.textContent="Speichern…";
  try{{
    const r=await fetch("/auth/reset-confirm",{{method:"POST",headers:{{"Content-Type":"application/json"}},body:JSON.stringify({{token:token,new_password:pw}})}});
    const data=await r.json().catch(()=>({{}}));
    if(r.ok){{
      msg.className="msg ok";msg.textContent="Passwort erfolgreich geändert. Weiterleitung zur Anmeldung…";
      setTimeout(()=>{{location.href="/app";}},1500);
    }}else{{
      msg.className="msg err";msg.textContent=(data.detail||"Reset fehlgeschlagen. Link evtl. abgelaufen.");
...
@limiter.limit("5/minute")
def auth_reset_confirm(request: Request, body: dict = Body(...)):
    """Verify reset token + apply new password.
    Token format: same as access JWT (1h expiry). Generated by /auth/reset-password."""
    token = (body.get("token") or "").strip()
    new_password = body.get("new_password") or ""
    if not token:
        err(400, "Token fehlt")
    if len(new_password) < 8:
        err(400, "Passwort muss mindestens 8 Zeichen haben")
    if not any(c.isupper() for c in new_password):
        err(400, "Passwort muss mindestens 1 Großbuchstaben enthalten")
    if not any(c.isdigit() for c in new_password):
        err(400, "Passwort muss mindestens 1 Zahl enthalten")
    try:
        from autotax.auth import decode_token as _decode
        payload = _decode(token)
        user_id = payload.get("sub")
        if not user_id:
            err(401, "Ungültiger Token")
    except Exception:
        err(401, "Ungültiger oder abgelaufener Reset-Link. Bitte erneut anfordern.")
    db = SessionLocal()
    try:
        u = db.query(User).filter(User.id == int(user_id)).first()
        if not u:
            err(404, "Benutzer nicht gefunden")
        u.hashed_password = hash_password(new_password)
        db.commit()
        logger.info("Password reset CONFIRMED for user_id=%s email=%s", user_id, _mask_email(u.email))
        return {"success": True, "message": "Passwort geändert."}
    except HTTPException:
        raise
    except Exception:
        logger.exception("Reset confirm failed")
...
def reset_password(request: Request, body: dict = Body(...)):
    ...
    Generic response (don't reveal email existence) for security.
    """
    ...
    try:
        u = db.query(User).filter(User.email == email).first()
        if not u:
            # Don't reveal if email exists — same response regardless
            logger.info("Password reset requested for non-existent email: %s", _mask_email(email))
            return {"success": True, "message": "Falls ein Konto existiert, wurde ein Reset-Link gesendet."}

        # Generate short-lived reset token (1 hour via create_access_token default)
        reset_token = create_access_token(u.id, u.email)
        public_base = (os.getenv("PUBLIC_APP_URL") or "https://autotax.cloud").rstrip("/")
        reset_url = f"{public_base}/auth/reset?token={reset_token}"

        # Send via Resend
        resend_key = (os.getenv("RESEND_API_KEY") or "").strip()
        if not resend_key:
            logger.error("Password reset email FAILED: RESEND_API_KEY env missing")
            # Return success to client anyway (don't leak config state)
            return {"success": True, "message": "Falls ein Konto existiert, wurde ein Reset-Link gesendet."}

        sender = (os.getenv("RESEND_FROM") or "AutoTax <noreply@autotax.cloud>").strip()
        subject = "AutoTax — Passwort zurücksetzen"
        # Plain, no-tracking HTML. Inline CSS only.
...
<p style="font-size:11px;color:#9ca3af;margin-top:24px">AutoTax-Cloud · Hüseyin Hancer · Saarbrücken<br>Diese E-Mail wurde automatisch generiert.</p>
        ...
        try:
            ...
            if r.status_code in (200, 201, 202):
                ...
                logger.warning("Resend password-reset returned %d: %s", r.status_code, r.text[:200])
        except Exception:
